...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check the Genesys2 board-native DWARF source-line external summary.")
    ...
    if not path.is_file():
        print(f"[FAIL] missing board-native source-line summary: {path}", file=sys.stderr)
        return 1
    errors = validate_external_summary(RECORD_ID, load_json(path), root)
    if errors:
        print("[FAIL] board-native source-line summary is not acceptable")
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"[PASS] board-native source-line summary accepted: {path}")
    return 0
