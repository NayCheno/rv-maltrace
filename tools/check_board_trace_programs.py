...
DEFAULT_DOC = Path("docs/board_trace_validation.md")
...
def check_doc(path: Path, board_doc: Path) -> list[str]:
    ...
    if "docs/board_trace_validation.md" not in board_text:
...
""",
    ...
    (root / DEFAULT_BOARD_DOC).write_text(
        "Phase 5.3 is tracked in docs/board_trace_validation.md and board/trace_validation/manifest.json.\n",
        encoding="utf-8",
    )
