...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check the Genesys2 board benign-control external summary.")
    ...
    if not path.is_file():
        print(f"[FAIL] missing board benign-control summary: {path}", file=sys.stderr)
        return 1
    errors = validate_external_summary(RECORD_ID, load_json(path), root)
    if errors:
        print("[FAIL] board benign-control summary is not acceptable")
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"[PASS] board benign-control summary accepted: {path}")
    return 0
