...
DEFAULT_RUNBOOK = Path("docs/baseline_bringup_runbook.md")
...
def check_board_doc(path: Path) -> list[str]:
    text = path.read_text(encoding="utf-8")
    errors: list[str] = []
    if "docs/baseline_bringup_runbook.md" not in text:
...
def write_good_fixture(root: Path) -> None:
    ...
    board_lines = [
        ...
        "",
        "Runbook: docs/baseline_bringup_runbook.md",
        "",
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        board_doc.write_text(board_doc.read_text(encoding="utf-8").replace("docs/baseline_bringup_runbook.md", ""), encoding="utf-8")
        errors = run_checks(root, DEFAULT_RUNBOOK, DEFAULT_BOARD_DOC)
