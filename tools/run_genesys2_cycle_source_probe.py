...
from check_genesys2_cycle_source_probe import SUMMARY_SCHEMA, parse_probe_log
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        return 0
    source, binary = load_checked_build_artifacts(build_manifest, label="cycle-source")
    ...
    return report_summary_exit(
        summary,
        args.summary,
        pass_message=f"[PASS] parsed {summary['row_count']} kernel-perf cycle-source rows",
    )
