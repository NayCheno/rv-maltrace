...
SAMPLES = {
    "hello_write": "e0000a01",
    "file_open_read_write": "e0000a02",
    "fork_exec": "e0000a03",
...
def program_command(sample_id: str, rep_name: str, runtime_root: str) -> str:
    ...
    return (
        f"printf 'RVMT_P0_BRAM_START sample={sample_id} rep={rep_name}\\n'; "
        f"{runtime_path}; "
        "rc=$?; "
        f"printf 'RVMT_P0_BRAM_DONE sample={sample_id} rep={rep_name} rc=%s\\n' \"$rc\""
    )
...
def package_command(args: argparse.Namespace) -> list[str]:
    return [
        sys.executable,
        "tools/package_genesys2_p0_bram_trace.py",
        "--run-root",
        str(args.run_root),
        "--minimum-repetitions",
...
def run(args: argparse.Namespace) -> int:
    ...
    print(f"[RUN] package P0 BRAM summary: {' '.join(command)}", flush=True)
    result = subprocess.run(command, cwd=args.root)
    if result.returncode != 0:
        return result.returncode
    print("[PASS] P0 BRAM repetitions complete")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Capture repeated P0 BRAM marker windows on Genesys2/CVA6.")
    parser.add_argument("--root", type=Path, default=Path.cwd())
    parser.add_argument("--run-root", type=Path, default=DEFAULT_RUN_ROOT)
    parser.add_argument("--runtime-root", default="/tmp/rvmt_p0")
    ...
    if args.start_repetition < 1:
        parser.error("--start-repetition must be >= 1")
    if args.start_repetition > args.repetitions:
        parser.error("--start-repetition must be <= --repetitions")
    if args.minimum_repetitions < 1:
        parser.error("--minimum-repetitions must be >= 1")
    return run(args)
