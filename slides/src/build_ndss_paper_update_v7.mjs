...
const PREVIEW = path.join(OUT, "preview-v7");
const LAYOUT = path.join(OUT, "layout-v7");
const WEB = path.join(OUT, "web-v7");
const W = 1280;
const H = 720;
...
const visualAudit = [
  ["01", "Right-side decorative bars looked like unresolved placeholders; artifact root was too close to the bottom.", "Removed decorative bars, added a three-step meeting objective stack, and lifted the artifact root into the safe area."],
  ["02", "Six long rows were readable but visually uniform; Q labels were small after Google Slides import.", "Kept the ordered checklist, enlarged labels, and tightened row rhythm so the page scans as six questions."],
  ["03", "The two cards left too much dead space under the core gap.", "Raised the cards and added stronger left/right contrast around semantics vs. trace."],
  ["04", "Three motivation cards were balanced but body copy could crowd the bottom of each card.", "Slightly increased card height and body lane width."],
  ["05", "The conclusion box and support rows competed for emphasis.", "Kept the conclusion as the dominant block and softened the lower rows."],
  ["06", "Evidence map rows needed clearer lane alignment.", "Widened label lane and text lane; aligned row dividers."],
  ["07", "Pipeline arrows could read as crossing through nodes.", "Kept connectors behind nodes and added more vertical separation before the source-label rows."],
  ["08", "Trace-correctness rows were clean but a little low-density.", "Adjusted row spacing for a stronger evidence-list rhythm."],
  ["09", "The 2x2 source-label cards were close to the red footer.", "Raised cards and increased footer separation."],
  ["10", "The red limitation card carried the most important caveat but was visually equal to support cards.", "Kept the limitation red and slightly stronger in body text."],
  ["11", "Hero metric was effective; supporting numbers needed more breathing room.", "Expanded right-side fact spacing and kept 122 as the focal point."],
  ["12", "Workload bars had uneven visual weight.", "Standardized bar widths and note alignment."],
  ["13", "Metric cards fit; footer needed more bottom margin.", "Lifted the footer and kept the warning readable."],
  ["14", "Boundary rows were clear but the last row was dense.", "Widened text lane and kept the red warning row inside the safe margin."],
  ["15", "Alternative explanations were the densest main slide.", "Reduced text size only slightly, widened text lane, and increased line spacing."],
  ["16", "Resource bars were clear; bottom warning was close to the slide edge.", "Lifted warning line and centered it under the bars."],
  ["17", "Decision slide had four dense rows.", "Widened text lane and made the verbs the visual anchor."],
  ["18", "Reproducibility slide mixed evidence and manuscript status.", "Aligned rows and kept manuscript polish as the only amber line."],
  ["19", "Takeaway page felt too sparse after dense evidence pages.", "Changed it to a five-step wrap-up so it mirrors the six-question arc."],
  ["20", "Streaming/DMA appendix was readable but had unnecessary vertical slack.", "Adjusted row spacing and kept it clearly marked as future work."],
  ["21", "Long sample names risked wrapping in Google Slides.", "Reduced roster body size and widened the text lane."],
  ["22", "Resource detail table had weak separation between header and body.", "Added stronger row spacing and kept numeric columns aligned."],
];
...
function header(slide, s) {
  ...
  text(slide, s.title, 72, 86, 1040, 58, { size: 34, bold: true, color: C.ink });
  rect(slide, 72, 176, 58, 4, C.blue);
  text(slide, s.claim, 160, 158, 925, 58, { size: 21, bold: true, color: C.body });
}
...
function cover(slide, s) {
  ...
  text(slide, "artifact root: results/evaluation/genesys2-cva6/current", 78, 626, 700, 26, { size: 14, color: C.white, mono: true });
  rect(slide, 910, 150, 220, 220, C.white);
  text(slide, "TRACE\nEVENTS\nFIRST", 910, 188, 220, 124, { size: 36, bold: true, color: C.blue, align: "center" });
  const steps = [["problem", 420, 205], ["evidence", 476, 170], ["decision", 532, 210]];
  steps.forEach(([label, y, w], i) => {
    rect(slide, 910, y, w, 28, i === 0 ? C.black : C.white);
    text(slide, label, 924, y + 7, w - 28, 12, { size: 10, bold: true, color: i === 0 ? C.white : C.blue, mono: true });
  });
}
...
function twoColumn(slide, left, right, y = 275) {
  ...
  for (const [x, b, color, fill] of blocks) {
    rect(slide, x, y - 18, 460, 224, fill, color, 2);
    text(slide, b.kicker, x + 28, y + 10, 360, 26, { size: 15, bold: true, color, mono: true });
    text(slide, b.title, x + 28, y + 58, 360, 42, { size: 28, bold: true, color: C.ink });
    text(slide, b.body, x + 28, y + 120, 392, 52, { size: 19, color: C.muted });
  }
}
...
function threeBlocks(slide, rows, y = 268) {
  rows.forEach((r, i) => {
    const x = 90 + i * 385;
    rect(slide, x, y, 330, 238, r.fill, r.color, 2);
    ...
    text(slide, r.body, x + 24, y + 150, 270, 56, { size: 17.5, color: C.muted });
  });
}
...
function rowList(slide, rows, y0 = 265, opts = {}) {
  ...
  rows.forEach((r, i) => {
    const y = y0 + i * step;
    rect(slide, labelX, y, labelW, 32, r.color || C.blue);
    text(slide, r.label, labelX, y + 7, labelW, 16, { size: 11.5, bold: true, color: C.white, align: "center", mono: true });
...
function sixQuestionMap(slide) {
  ...
  rowList(slide, rows, 214, { step: 64, labelW: 92, textX: 246, textW: 875, textSize: 19.5, textHeight: 34, lineOffset: 50 });
}
...
function sourceLabels(slide) {
  ...
  rows.forEach(([label, body, color], i) => {
    const x = 105 + (i % 2) * 540;
    const y = 252 + Math.floor(i / 2) * 145;
  ...
  text(slide, "The point is not more labels; it is preventing reference evidence from being reported as hardware recovery.", 170, 565, 940, 34, { size: 20, bold: true, color: C.red, align: "center" });
}

function alternatives(slide) {
  rowList(slide, [
  ...
  ], 238, { labelW: 122, textX: 286, textW: 865, textSize: 20.5, textHeight: 46, lineOffset: 64, step: 84 });
}

function nextDecision(slide) {
  rowList(slide, [
    ...
    { label: "REMOVE", text: "Move real-malware accuracy and payload-equivalence claims to limitations/future work", color: C.red },
...
function thesis(slide) {
  ...
  text(slide, "SOURCE LABELS", 695, 282, 470, 58, { size: 42, bold: true, color: C.green, mono: true });
  ...
  text(slide, "Source labels mark where derived semantics came from.", 700, 430, 450, 50, { size: 23, bold: true, color: C.ink });
}
...
function paperStory(slide) {
  ...
  steps.forEach(([a, b], i) => {
    const x = 82 + i * 230;
    const color = i === 4 ? C.red : i === 3 ? C.green : C.blue;
    const fill = i === 4 ? C.redPale : i === 3 ? C.greenPale : C.bluePale;
    rect(slide, x, 308, 196, 140, fill, color, 2);
    text(slide, a, x + 18, 338, 154, 28, { size: 19, bold: true, color });
    text(slide, b, x + 18, 388, 150, 38, { size: 16.5, bold: true, color: C.ink });
  });
}
...
const renderers = {
  sixQuestionMap, whyMatters, currentConclusion, evidenceMap, sourceLabels,
  alternatives, nextDecision,
...
async function writeDocs() {
  const auditDoc = [
    "# Slide Format Audit for V7",
    "",
    "V7 applies a page-by-page format pass to the weekly NDSS update deck. Each correction is tied to what the slide must prove in a doctoral weekly meeting: problem, value, conclusion, evidence, boundary, or next decision.",
    "",
  ...
  await fs.writeFile(path.join(OUT, "slide_format_audit_v7.md"), auditDoc, "utf8");
  await fs.writeFile(path.join(OUT, "slide_visual_audit_v7.md"), auditDoc, "utf8");

  await fs.writeFile(path.join(OUT, "terminology_audit_v7.md"), [
    "# RV-MalTrace Terminology Audit V7",
    "",
  ...
  await fs.writeFile(path.join(OUT, "outline_v7.md"), [
...
async function build() {
  ...
  await pptx.save(path.join(OUT, "ndss_paper_update_v7.pptx"));
  const inspect = await presentation.inspect({ kind: "slide,textbox,shape,notes,layout", maxChars: 16000 });
  await fs.writeFile(path.join(OUT, "ndss_paper_update_v7.pptx.inspect.ndjson"), inspect.ndjson, "utf8");
}
