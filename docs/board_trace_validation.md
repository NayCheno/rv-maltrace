...
Phase 5.3 defines the first trace-enabled Genesys 2 validation program set.

This document is a run plan and does not claim the programs have passed on
hardware. Board evidence must be captured under:
...
Each program directory under the board run must contain:
