...
Our insight is that hardware trace events can be connected to OS-level behavior
semantics when each reconstructed field is explicitly labeled with its provenance
source, enabling auditable behavior reconstruction without overstating hardware
evidence.
...
\begin{itemize}
  ...
  PIE/ASLR, dynamic loading, fork/exec, and stripped binaries, achieving [X\%]
  coverage on controlled workloads.
  \item A reproducible artifact package with manifests and SHA-256 checks that
  enables independent verification of all reported results.
\end{itemize}

We focus on controlled RISC-V Linux workloads. Board cycle-overhead measurement
and production streaming transport are left for future work.
