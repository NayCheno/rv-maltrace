...
This ensures the trace is transparent to software: the workload cannot
detect, evade, or interfere with the trace adapter.
Because the adapter observes only committed architectural state, the
target process has no mechanism to discover whether tracing is active
or to alter its behavior based on trace presence.
