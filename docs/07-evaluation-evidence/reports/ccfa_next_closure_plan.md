...
| Field | Current value |
| --- | --- |
| allowed claims | Controlled Genesys2/CVA6 P0 and safe-surrogate BRAM marker-window evidence, bounded-prefix hardware ARG_MEM coverage for openat/write/execve, full hardware pointer-string readiness with the full-string external blocker still open, controlled repetition/statistical robustness audit, cycle-normalized streaming/DMA target baseline for future throughput experiments, future non-BRAM streaming/DMA transport readiness, local code/process attribution summaries, board-native DWARF/source-line and board benign-control external summaries retained as invalid/open blockers, per-sample controlled case-study packages, lightweight artifact package / fresh-clone reproduction commands, external closure readiness/intake/plan/preflight contracts, external operator handoff packet, non-evidence external summary templates, and malware-like behavior audit over safe surrogate workloads. |
...
| run id | `20260702-current-p0-cohort`; `20260702-current-safe-surrogate-cohort`; `20260612-pointer-snapshot-bram`; `current-quality-20260612` |
| board / CPU / bitstream hash | Board Digilent Genesys2; CPU CVA6 rv64gc sv39; P0 BRAM bitstream `04dbd5bf4b661427628bd2142e619c2b2526b7fd0598ffeb6df954fc5811b354`; P0 LTX `16360c1f20031509b61dafb4847ba638b85d1a758a1c10b96d2faacdb4ce7ee3`; safe BRAM bitstream `04dbd5bf4b661427628bd2142e619c2b2526b7fd0598ffeb6df954fc5811b354`; safe BRAM LTX `16360c1f20031509b61dafb4847ba638b85d1a758a1c10b96d2faacdb4ce7ee3` |
| command transcript | `results/board/genesys2_trace_validation/20260702-current-p0-cohort/*/rep_*/capture.log`; `results/board/genesys2_trace_validation/20260702-current-p0-cohort/*/rep_*/uart.log`; `results/board/genesys2_trace_validation/20260612-pointer-snapshot-bram/*/rep_*/capture.log`; `results/board/genesys2_trace_validation/20260702-current-safe-surrogate-cohort/*/rep_*/capture.log` |
...
Official-image addendum: the official SD-image strict-SRET/P0 provenance root is `results/board/genesys2_trace_validation/20260702-current-p0-cohort`, captured with trace-marker bitstream SHA256 `04dbd5bf4b661427628bd2142e619c2b2526b7fd0598ffeb6df954fc5811b354`. This is tied into the current checks through `strict_sret_board_smoke_summary.json` and `p0_bram_trace_summary.json`, while dated roots remain provenance only.
...
- Review closure audit: `review_closure_audit.json` and `ccfa_review_closure_audit.md` map the original 2026-06 CCF-A review requirements to current evidence, objective exclusions, checker commands, and the four still-open external intake records.
...
- Board-native DWARF/source-line attribution, scoped full hardware pointer strings, and Genesys2 board benign-control false-positive evidence remain external/open. The current local package retains readiness and present-invalid candidate summaries only; it does not accept those summaries as completion evidence.
...
- `external_closure_intake.json` makes future board/RTL summary intake machine-checkable. Board-native DWARF/source-line, full hardware pointer strings, production streaming/DMA, and Genesys2 board benign-control summaries all remain present-invalid/open and cannot be claimed closed yet.
...
- Hardware pointer claims are bounded-prefix ARG_MEM plus readiness for future full-string collection; the candidate `hardware_pointer_strings_summary.json` remains present-invalid/open external intake material.
