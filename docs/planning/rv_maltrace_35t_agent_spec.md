...
若仓库没有 `docs/results/`，可以创建。若已有更合适结果目录，使用仓库现有规范，但必须在最终回答中说明实际路径。
...
1. 创建或更新 docs/results/rv_maltrace_35t_application_closure.md。
...
5. 创建或更新 docs/results/rv_maltrace_35t_application_case_studies.md，至少覆盖 illegal_trap、process_chain、dynamic_executable_memory；建议增加 file_scan 或 batch_open_read_write。
