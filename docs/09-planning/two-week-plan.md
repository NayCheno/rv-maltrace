...
2026-05-17 复核已重新跑 `sim:trace-unit`、`sim:cva6-smoke`、`sim:cva6-full-soc`，重新生成 `results/vivado_sim/summary.json`，并复跑 semantic recovery、fuzz checker、compression、timing、trace-boundary、board-minimal、evaluation 等 Python gate。
...
| --- | --- | --- | --- | --- |
| WP0 baseline freeze | 冻结当前可复现实验基线 | `docs/07-evaluation-evidence/reports/sim_results.md`、`results/vivado_sim/` | `summary.json`、per-test logs、baseline notes | `uv run rvmt sim:summary` |
...
| WP5 lightweight analysis | 证明 event-selective trace 不是 full trace | `docs/02-trace-architecture/trace_format.md`、`tools/compress_trace.py` | compression stats、filter/drop report | `uv run python tools/compress_trace.py ... --check-roundtrip --stats` |
| WP6 evaluation/resource/board gate | 冻结论文级评估与上板准入 | `docs/07-evaluation-evidence/evaluation_plan.md`、`docs/07-evaluation-evidence/reports/resource_report.md`、`docs/03-platform-architecture/genesys2/` | RQ gate、resource delta plan、board go/no-go checklist | `uv run python tools/check_evaluation_plan.py` |
...
| `docs` | `docs/07-evaluation-evidence/reports/sim_results.md` 与实际结果一致 |
...
`docs/09-planning/next-plan.md` 已指出几个关键坑：ECALL 不能只从 normal retire path 捕获，compressed instruction 不能默认 `next_pc = pc + 4`，JALR target 要优先用 core resolved target/next_pc，`a0-a7 shadow` 也有 stale 风险。
...
`docs/02-trace-architecture/trace_format.md` 已经有 filter controls 和 compressed trace prototype，包含 `cycle_delta`、`pc_delta`、event-specific payload、context delta 等设计。
...
`docs/07-evaluation-evidence/evaluation_plan.md` 已经把研究问题拆成 correctness、semantic reconstruction、low perturbation、evasion resistance、hardware cost、malware behavior usefulness，并列出 `strace`/`ptrace`、eBPF-only、QEMU plugin、software instrumentation、event-only、pointer snapshot、kernel helper 等 baseline。
...
`docs/07-evaluation-evidence/reports/resource_report.md` 当前已有 Genesys 2 baseline routed utilization/timing snapshot、trace queue/drop 统计，以及 `build/vivado/genesys2-cv64a6_imafdc_sv39-trace` 生成的 trace-enabled routed delta。当前 delta：LUT +40,794 (+48.04%)、FF +2,810 (+4.97%)、BRAM/DSP 无变化、报告 slack 与 baseline 持平。
...
- 若 `summary.json` 与文档不一致，先更新 `docs/07-evaluation-evidence/reports/sim_results.md`，不要继续扩功能。
...
- watched CSR 列表和优先级必须写在 `docs/02-trace-architecture/signal_map.md` 或相关文档中。
...
- backpressure test 的 drop rows 与 `docs/07-evaluation-evidence/reports/resource_report.md` 中 drop summary 一致。
...
- `docs/07-evaluation-evidence/reports/resource_report.md` 与最新 `summary.json` 的 drop rows 对齐。
...
- trace-enabled full FPGA build 的 LUT/FF/BRAM/slack delta 已由 `uv run rvmt bitstream:build-trace` 和 `uv run python tools/generate_resource_report.py` 记录。
...
- 文档中不得把 baseline Genesys 2 routed report 写成 trace-enabled resource overhead；trace-enabled overhead 必须引用 `build/vivado/genesys2-cv64a6_imafdc_sv39-trace/reports/`。
- 若 trace-enabled implementation report 缺失，只能写“baseline routed snapshot + simulation drop stats”；当前本机报告已生成。
...
- `docs/07-evaluation-evidence/evaluation_plan.md` 仍保持 TODO，除非真实 artifact 已存在。
...
docs/09-planning/two-week-plan.md
docs/03-platform-architecture/genesys2/board_trace_minimal.md
...
| CVA6 LSU hook TBD | `ARG_MEM` 只做 synthetic pointer snapshot，不宣称 Linux pointer recovery | `docs/02-trace-architecture/signal_map.md` 明确 mem load hook TBD |
...
| trace-enabled resource delta 缺失 | 2026-05-17 已生成 trace-enabled Genesys 2 routed report 并写入 `docs/07-evaluation-evidence/reports/resource_report.md` | 后续改 trace RTL 必须重跑 `uv run rvmt bitstream:build-trace` |
...
| 标准 | 状态 | 证据或边界 |
| --- | --- | --- |
| Trace event schema frozen | PASS | `docs/02-trace-architecture/trace_format.md` 覆盖当前 JSONL/event schema；`sim:summary` 生成 PASS summary |
...
| U-mode syscall entry + SRET-to-U syscall return | PASS | `syscall_ret` 和 `rvfi_adapter` 覆盖 pairing、return value、return PC、duration |
| bounded `ARG_MEM` pointer snapshot synthetic tests | PASS | `pointer_string`、`pointer_guardrails` PASS；仍不是 Linux pointer recovery PASS |
